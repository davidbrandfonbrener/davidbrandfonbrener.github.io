...
This post will go through an example that motivates what happens in this problem and introduce some of the main results from our recent paper [Offline Contextual Bandits with Overparameterized Models](https://arxiv.org/abs/2006.15368). Briefly, we find that policy-based algorithms can struggle with serious overfitting problems because their objectives are not *action-stable*, but value-based algorithms effectively leverage overparameterized models and are *action-stable*. Where action-stability is a property that we introduce that says that at each state there exists a prediction (action distribution or action-value vector) which is optimizes the algorithm's objective no matter which action is observed in the data.
